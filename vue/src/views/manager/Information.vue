<template>
    <div>
        <div class="search">
            ...
            <el-select v-model="status" placeholder="请选择审核状态" style="width: 200px; margin-left: 5px">
        ...
        <div class="table">
            <el-table :data="tableData" stripe>
                ...
                <el-table-column prop="author" label="上传用户"></el-table-column>
        ...
        <el-dialog title="课程内容" :visible.sync="editorVisible" width="50%" :close-on-click-modal="false"
            destroy-on-close>
...
export default {
    ...
    methods: {
        ...
        save() {   // 保存按钮触发的逻辑  它会触发新增或者更新
            if(this.form.status==='审核通过'){
                console.log(this.form.id);
                this.$request.get('/score/selectById/'+this.form.id).then(res=>{
                    console.log(res);
                    if(res.data.status!=='审核通过'){
                        console.log(res);
                        console.log(this.form.author);
                        this.$request.get('/user/updateByUsername',
                        {params: {username: this.form.author}}).then(res=>{
                            if(res.code==='200'){
